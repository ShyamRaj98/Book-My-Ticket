...
export const fetchProfile = createAsyncThunk('auth/fetchProfile', async (_, thunkAPI) => {
  ...
  if (!token) return thunkAPI.rejectWithValue({ error: 'no token' });
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(register.rejected, (s, a) => { s.loading = false; s.error = a.payload?.error || 'register failed'; })
      ...
      .addCase(login.rejected, (s, a) => { s.loading = false; s.error = a.payload?.error || 'login failed'; })
      ...
      .addCase(fetchProfile.rejected, (s, a) => { s.loading = false; s.error = a.payload?.error || 'fetch failed'; });
  }
});
